...
import ModalGenerateEmotion from "../components/modal/modal_generate_emotion";
import funCekEmotionBatch from "../fun/cek_emotion_batch";

export default function ViewGenerateEmotion({ param, provinsi, kabupaten, candidate }: { param: any, provinsi: any, kabupaten: any, candidate: any }) {
   ...
   async function onCek() {
      ...
      const cek = await funCekEmotionBatch({ candidates: candidateIds, dates: dateList })
   ...
   return (
      <>
         <Stack>
            <Text fw={"bold"}>GENERATE DATA EMOSI KANDIDAT</Text>
         </Stack>
         <Box>
            ...
            </Paper>
            {
               isValid && (
                  <Paper shadow="xs" p="xl" mt={"lg"}>
                     ...
                     <Text mb={20} c={"dimmed"}>
                        {genCandidates.length} kandidat × {genDates.length} tanggal ({moment(genDates[0]).format('DD/MM/YYYY')} - {moment(genDates[genDates.length - 1]).format('DD/MM/YYYY')}). Nilai sentimen akan dibuat otomatis secara acak dan berbeda tiap tanggal & kandidat.
                     </Text>
         ...
         >
            <Alert color="gray" variant="outline">
               <Text fw={700} ta={"center"} mb={20} mt={20}>
                  DITEMUKAN DATA PADA {overwriteCount} KOMBINASI KANDIDAT & TANGGAL. MELANJUTKAN AKAN MENGHAPUS DATA LAMA TERSEBUT. LANJUTKAN?
               </Text>
         ...
         <Modal
            ...
            closeOnClickOutside={false}
         >
            <ModalGenerateEmotion candidates={genCandidates} dates={genDates} provinsi={isProvinsi} onSuccess={() => {
               setValid(false)
            }} />
         </Modal>
      </>
